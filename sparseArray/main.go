...
func matchingStrings(in []string, queries []string) []int32 {
	result := make([]int32, len(queries))
	count := 0
	for i := 0; i < len(queries); i++ {
		for j := 0; j < len(in); j++ {
			if strings.Compare(queries[i], in[j]) == 0 {
				count += 1
			}
		}
		result[i] = int32(count)
		count = 0
	}
	return result
}
...
func main() {
	...
	res := matchingStrings(in, queries)
